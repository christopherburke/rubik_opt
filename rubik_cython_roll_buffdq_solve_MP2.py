...
#                15:"BC", 16:"BG", 17:"BH"}
    ...
    #print(init_faceids)
    # Hardcode the faceids calculated like from rubik_cube_debugpath routine
    # 17 turn 7 minutes (Botto @0.62 hrs)
#    init_faceids = [9, 32, 29, 68, 13, 41, 24, 76, 25, 40, 12, 61, 21, 64, 1, 52, 2, 65, 20, 72, 6, 36, 18, 44, 16, 37, 5, 56, 30, 33, 8, 49, 26, 53, 0, 45, 17, 48, 10, 77, 28, 57, 4, 73, 22, 60, 14, 69]
    # 18 turn XX min (@10.5 hr)
#    init_faceids = [12, 68, 1, 60, 16, 52, 20, 37, 21, 53, 18, 40, 8, 76, 24, 56, 25, 77, 10, 65, 4, 44, 30, 48, 28, 45, 6, 32, 2, 69, 14, 73, 22, 57, 26, 49, 29, 72, 13, 36, 0, 33, 5, 64, 9, 41, 17, 61]
    init_faceids = [14, 64, 1, 76, 18, 52, 24, 61, 25, 53, 17, 73, 21, 44, 6, 57, 4, 45, 20, 68, 10, 49, 30, 40, 28, 48, 9, 33, 2, 65, 13, 36, 26, 56, 5, 41, 29, 37, 12, 60, 0, 32, 8, 69, 22, 72, 16, 77]
    ...
    print('Start makeing DBs shared')
...
#    retval = rcm.DFS_cython_solve(bytes(init_faceids), 0, cornerDB, edgeDB, edge1DB, edge2DB)
    ...
    for i in range(18):
        ...
        # Adjust the move number to put the second move
    ...
    for MAXDELDEP in np.arange(5,largest_MAXDELDEP+1):
        ...
        if not retval == 2:
            ...
            for i, curnewmoves in enumerate(newmoves2):
                ...
                holdarr = array.array('i', holdlist) # this wasnt actually necessary
                        # as the array is converted to list by worker
                        # but left it here could be removed 
                work_args.append(holdarr)
                
            pool = Pool(processes = USENCPUS)
            results = pool.map(child, work_args)
